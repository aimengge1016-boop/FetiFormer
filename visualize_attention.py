...
def visualize_attention_heatmap(attention_matrix, seq_len, save_path, title="Query-Key Attention Heatmap"):
    ...
    if attention_matrix.ndim == 4:
        ...
        data = attention_matrix[0, 0, :seq_len, :seq_len]
    elif attention_matrix.ndim == 3:
        data = attention_matrix[0, :seq_len, :seq_len]
    else:
        data = attention_matrix[:seq_len, :seq_len]
    
    im1 = _draw_heatmap(data, ax1)
    ax1.set_title(f'Head 1 Attention\n(First Batch)', fontsize=12)
    ax1.set_xlabel('Key Position (Time Steps)')
    ax1.set_ylabel('Query Position (Time Steps)')
    ...
    if attention_matrix.ndim == 4:
        ...
        data = attention_matrix[0, :, :, :].mean(axis=1)
        data = data[:seq_len, :seq_len]
        n_heads = attention_matrix.shape[2]
    elif attention_matrix.ndim == 3:
        # [H, N, N] -> 对所有 heads 取平均
        data = attention_matrix.mean(axis=0)[:seq_len, :seq_len]
        n_heads = attention_matrix.shape[0]
    else:
        data = attention_matrix[:seq_len, :seq_len]
        n_heads = 1
    
    im2 = _draw_heatmap(data, ax2)
    ax2.set_title(f'Average Multi-Head Attention\n(All {n_heads} Heads)', fontsize=12)
    ax2.set_xlabel('Key Position (Time Steps)')
    ax2.set_ylabel('Query Position (Time Steps)')
    ...
    if attention_matrix.ndim == 4:
        ...
        data = attention_matrix[0, :, :, :].mean(axis=1)  # 对 heads 维度取平均
        data = data[:seq_len, :seq_len]
    elif attention_matrix.ndim == 3:
        # [H, N, N] -> 对所有 heads 取平均
        data = attention_matrix.mean(axis=0)[:seq_len, :seq_len]
    ...
    # 计算每个query位置的平均注意力权重
    mean_attn_per_query = data.mean(axis=1)
    std_attn_per_query = data.std(axis=1)
    ...
    ax3.set_xlabel('Query Position (Time Steps)')
...
def visualize_attention_advanced(attentions, seq_len, save_path):
    ...
    for l in range(1, n_layers):
        diff = np.abs(all_attns[l] - all_attns[l-1]).mean()  # scalar
        layer_diff.append(diff)
    
    x = np.arange(1, n_layers)
    ax.plot(x, layer_diff, 'o-', color='blue', alpha=0.8, linewidth=2)
    ax.set_xlabel('Layer')
    ...
    for l in range(n_layers):
        ...
        print(f"DEBUG: layer {l}, layer_attn shape = {layer_attn.shape}, entropy shape = {entropy.shape}")
        entropy_per_query.append(entropy)
    entropy_per_query = np.array(entropy_per_query).mean(axis=0)
    print(f"DEBUG: final entropy_per_query shape = {entropy_per_query.shape}")
    
    x = np.arange(actual_seq_len)
    ax.plot(x, entropy_per_query, 'b-', linewidth=1.5)
    ax.fill_between(x, entropy_per_query, alpha=0.3)
    ...
    attention_coverage = all_attns.mean(axis=0)  # [N, N]
    coverage_per_key = attention_coverage.sum(axis=0)  # [N]
...
def save_individual_layer_heatmaps(attentions, output_dir, seq_len=None):
    ...
    for layer_idx in range(n_layers):
        ...
        save_single_heatmap_pdf(
            ...
            xlabel="Key Position (Variable Index)",
            ylabel="Query Position (Variable Index)"
        )
