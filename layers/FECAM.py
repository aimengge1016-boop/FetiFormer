...
class dct_channel_block(nn.Module):
    def __init__(self, channel):
        ...
        # dct_norm will be applied dynamically in forward using the last-dim size
        self.dct_norm = None
        # self.dct_norm = nn.LayerNorm([36], eps=1e-6)#for lstm on length-wise on ill with input =36


    def forward(self, x):
        b, c, l = x.size() # (B,C,L) where last dim l should equal 'channel' passed in init
        # y = self.avg_pool(x) # (B,C,L) -> (B,C,1)
        ...
        stack_dct = torch.stack(list, dim=1)  # [B, C, l]

        # Apply layer-norm dynamically over last dim
        stack_dct = torch.nn.functional.layer_norm(stack_dct, normalized_shape=(stack_dct.shape[-1],))

        lr_weight = self.fc(stack_dct)  # apply fc on last dim
        lr_weight = torch.nn.functional.layer_norm(lr_weight, normalized_shape=(lr_weight.shape[-1],))
