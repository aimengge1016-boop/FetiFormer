...
class FlashAttention(nn.Module):
    ...
    def flash_attention_forward(self, Q, K, V, mask=None):
        ...
        # Keep all buffers on the same device as Q (avoid hard-coding 'cuda')
...
class FullAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None, time_bias=None):
        ...
        # Optionally add time-based bias to attention scores.
        # time_bias expected shapes:
        #  - None: no bias
        #  - [B, H, 1] (per-head scalar) -> broadcast to [B, H, L, S]
        #  - [B, H, L, S] (full bias) -> added directly
        if time_bias is not None:
            if time_bias.dim() == 3:
                # [B, H, 1] -> [B, H, 1, 1] -> broadcast
                scores = scores + time_bias.unsqueeze(-1)
            elif time_bias.dim() == 4:
                scores = scores + time_bias
            else:
                # fallback: try to broadcast last dim
                try:
                    tb = time_bias.view(B, -1, 1, 1)
                    scores = scores + tb
                except Exception:
                    pass
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None, time_bias=None, time_context=None):
        B, L, _ = queries.shape
        _, S, _ = keys.shape
        H = self.n_heads
        ...
        # Forward optional time_bias and time_context to inner attention if supported.
        try:
